<?php
    ...
    $figuresInfo = $db->queryAll("SELECT `figure`.`id_figure` AS `id_figure`, `figure`.`name` AS `name`, `figure`.`image` AS `image`, `figure`.`plastic` AS `plastic`, `figure`.`print_time` AS `print_time`, `figure`.`filling_out` FROM `figure`, `figure_user` WHERE `figure_user`.`id_user` = $idUser AND `figure_user`.`id_figure` = `figure`.`id_figure`");
    ...
    for ($i = 0; $i < count($figuresInfo); $i++)
    {
        ...
        $figureGrades = $db->query("SELECT SUM(`grade`), COUNT(*) FROM `figure_review_grade` WHERE `id_figure` = $idFigure");
